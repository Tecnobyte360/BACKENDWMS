...
>
    ...
    class="bg-gray-600 text-white px-4 py-2 rounded-md hover:bg-gray-700 transition-colors inline-flex items-center justify-center"
>
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 flex space-x-2">
                                ...
                                <button @click="openModal($event)"
                                        ...
                                        wire:confirm="¿Estás seguro de borrar {{$role->name}}?"
...
<div 
    x-show="isVisibleCreateRolesModal"
    ...
    class="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm bg-black/40 dark:bg-gray-900/60"
>
    <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-md">
        <h2 class="text-2xl font-bold mb-5 text-gray-800 dark:text-white text-center">
            <i class="fa-solid fa-user-tag text-violet-600 mr-2"></i> Crear Nuevo Rol
        </h2>

        <div class="space-y-6">
            {{-- Nombre del Rol --}}
            <div class="relative">
                <label class="block text-sm font-semibold text-gray-600 dark:text-gray-300 mb-1">Nombre del Rol *</label>
                <input type="text" wire:model.lazy="name" oninput="this.value = this.value.toLowerCase();"
            ...
            <div class="relative">
                ...
                <textarea wire:model.lazy="description"
                    class="w-full px-4 py-3 rounded-2xl border transition-all shadow-inner
                    @error('description') border-red-500 focus:ring-red-300 focus:border-red-500
                    @else border-gray-300 focus:ring-violet-300 focus:border-violet-500 @enderror
                    dark:border-gray-700 dark:bg-gray-800 dark:text-white focus:ring-4"
                    rows="3"
                    placeholder="Describe brevemente el propósito del rol"></textarea>

                @if(!empty($description) && !$errors->has('description'))
                    <div class="absolute inset-y-0 right-4 flex items-center pointer-events-none">
                        <i class="fas fa-check-circle text-green-500 text-lg"></i>
                    </div>
                @endif
        ...
        <div class="mt-6 flex justify-end space-x-3">
            <button wire:click="CloseModalClick('isVisibleCreateRolesModal')" x-on:click="isVisibleCreateRolesModal = false"
                class="px-4 py-2 bg-gray-300 rounded-xl text-sm hover:bg-gray-400 transition">
                Cancelar
            </button>

            <button wire:click="store"
                class="px-5 py-2 bg-violet-600 text-white rounded-xl text-sm hover:bg-violet-700 shadow-md flex items-center gap-2 transition">
